...
export async function raiseAlert(
  ...
  try {
    await sendTrackedEmail('send_admin_alert', {
      ...
      subject: `${severity === 'critical' ? '🔴' : '⚠️'} CarHaki ${severity}: ${probe}`,
      html: `<p><strong>${probe}</strong> is failing.</p>
             <p>${detail}</p>
             <p>Raised ${new Date().toISOString()}. You will not be emailed again for this
             probe until it recovers. See <a href="https://carhaki.com/admin">the admin panel</a>.</p>`,
    });
...
export async function resolveAlert(probe: string): Promise<boolean> {
  ...
  try {
    await sendTrackedEmail('send_admin_alert', {
      from: process.env.RESEND_FROM_EMAIL || 'CarHaki <reports@carhaki.com>',
      to: process.env.ADMIN_EMAIL || 'support@carhaki.com',
      subject: `✅ CarHaki recovered: ${probe}`,
